...
class bank(gpay):
    ...
    def check_bank(self):
        self.__ubank = input("Enter your bank")
        self.__uaccno = input("Enter Bank Account number")
        if len (self.__uaccno) <=17:
            print("Succesfully Verified account number")
        else:
            raise TypeError("Check your bank account number")
    def pin_creation(self):
        self.__pin = input("Set up a 4 digit upi pin ")
        if len(self.__pin) == 4:
...
class userdata(bank):
    def __init__(self):
            self.__alldata=[self.__uname,self.__unumber,self.__umailid,self.__upi,self.__ubank,self.__uaccnos,self.__pin]
...
class payment(bank):
    ...
    def reciver_data(self,ifsc):
        ...
        if type(self.__rname) == str:
                self.__bankname= input("Enter Reciber's Bank :")
